...
st.caption(
    f"Pool de referencia de Lartigau: {reference_years} "
    ...
    "Las ocho series previas están identificadas sólo por año; no se atribuyen todas a Lartigau."
)
if seasonal_reference.attrs["source_2026"]["used"]:
    st.caption(
        f"La referencia 2026 usa el total registrado del 01/02 al {reference_2026_from:%d/%m/%Y}. "
...
with tab_audit:
    ...
    with st.expander("Curvas de la referencia local"):
        ...
        st.download_button(
            ...
            "lartigau_referencia_local.csv", "text/csv",
        )
    ...
    audit_summary = pd.DataFrame(
        {
            "Variable": [
                ...
                "Decaimiento desde 15/04", "Pool de referencia",
            ],
            "Valor": [
                ...
                f"{reference_years}; n={reference_campaigns}; excluye 2010, 2015, Balcarce, San Pedro y Tres Arroyos 2025",
            ],
        }
    )
